...
class TagsHelperPlugin extends DSCHelperPlugin
{
	 ...
	 */
	function getPluginsWithEvent( $eventName, $folder='Tags' )
	{
		$return = array();
		if ($plugins = TagsHelperPlugin::getPlugins( $folder ))
		{
			foreach ($plugins as $plugin)
			{
				if (TagsHelperPlugin::hasEvent( $plugin, $eventName ))
				{
					$return[] = $plugin;
				}
			}
	 ...
	 * @return array
	 */
	function getPlugins( $folder='Tags' )
	{
	...
	function hasEvent( $element, $eventName )
	{
	   ...
	   if (!$element || !is_object($element)) {
		...
		$import 	= JPluginHelper::importPlugin( strtolower('Tags'), $element->element );
		$dispatcher	= JDispatcher::getInstance();
