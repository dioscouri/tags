...
// TODO Make all Tags plugins extend this _base file, to reduce code redundancy

/** Import library dependencies */
jimport('joomla.plugin.plugin');
jimport('joomla.utilities.string');

class TagsPluginBase extends JPlugin
{
    /**
     ...
     * @access protected
     */
    function _getLayout($layout, $vars = false, $plugin = '', $group = 'tags' )
    {
        if (empty($plugin)) 
        {
    ...
    function _displayArticle()
    {
        ...
        if ($articleid)
        {
            Tags::load( 'TagsArticle', 'library.article' );
            $html = TagsArticle::display( $articleid );
        }
    ...
    function _getMe()
    {
        if (empty($this->_row))
        {
            ...
            $table = JTable::getInstance('Shipping', 'TagsTable');
            $table->load( array('element'=>$this->_element, 'folder'=>'tags') );
            $this->_row = $table;
        }
        return $this->_row;
    }
    ...
    /**
     * Make the standard Tags Tables avaiable in the plugin
     */
    protected function includeTagsTables()
    {
        // Include Tags Tables Classes
        JTable::addIncludePath( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_tags'.DS.'tables' );
    }
    
    /**
     * Include a particular Tags Model
     * @param $name the name of the mode (ex: products)
     */
    protected function includeTagsModel($name)
    {
        if (strtolower($name) != 'base')
            Tags::load( 'TagsModel'.ucfirst(strtolower($name)), 'models.'.strtolower($name) );
        else
     ...
     */
    protected function includeCustomModel($name, $plugin = '', $group = 'tags')
    {
        if (empty($plugin)) 
        {
            $plugin = $this->_element;
        ...
        if(!class_exists('TagsModel'.$name))
            JLoader::import( 'plugins.'.$group.'.'.$plugin.'.models.'.strtolower($name), JPATH_SITE );
    }
   
    /**
     * add a user-defined table to list of available tables (including the Tags tables
     * @param $plugin the name of the plugin in which the table is stored
     * @param $group the group of the plugin
     */
    protected function includeCustomTables($plugin = '', $group = 'tags'){
     ...
     * @param $group the group of the plugin
     */
    protected function includeCustomView($name, $plugin = '', $group = 'tags')
    {
        if (empty($plugin)) 
        {
            $plugin = $this->_element;
